...

{
    public class TagManagaer : MonoBehaviour
    {
        ...
        void procDetections(Point topLeft, Point topRight, Point bottomRight, Point bottomLeft)
        {
        ...
        public void calibrateCamera()
        {
            ...
            if (found)
            {
                ...
                for (int i = 0; i < boardSize.height; i++)
                {
                    ...
                    {
                        objPointsList.Add(new Point3(j * squareSize, i * squareSize, 0));
                    }
                }
            ...
            int max_d = (int)Mathf.Max(_webcamtexture.width(), _webcamtexture.height());
            double fx = max_d;
            double fy = max_d;
        ...
        {
            _webcamtexture = new Mat(camTextureManager.WebCamTexture.height, camTextureManager.WebCamTexture.width, CvType.CV_8UC1);
            _webcamtexture_grey = new Mat(camTextureManager.WebCamTexture.height, camTextureManager.WebCamTexture.width, CvType.CV_8UC1);
            _webcamtexture_blur = new Mat(camTextureManager.WebCamTexture.height, camTextureManager.WebCamTexture.width, CvType.CV_8UC1);


            _previewTexture = new Mat(camTextureManager.WebCamTexture.height, camTextureManager.WebCamTexture.width, CvType.CV_8UC1);
            ...
            _previewTexture = _webcamtexture;
            

            arucoDetector.detectMarkers(_webcamtexture_grey, DetectedMarkerPos, DetectedMarkerID, rejectDetectedMarkerPos);
            ...
            if (DetectedMarkerID.total() > 0)
            {
                for (int i = 0; i < DetectedMarkerID.total(); i++)
                {
                    ...
                    Debug.Log($"Detected AprilTag ID: {markerId}: Top-Left: {topLeft}, Top-Right: {topRight}, Bottom-Right: {bottomRight}, Bottom-Left: {bottomLeft}");
